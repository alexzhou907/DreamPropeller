...
class SingleImageDataModuleConfig:
    ...
    eval_bg_color: Any = None

    num_workers: int = 8


def load_images(cfg, img_size):
    width, height = img_size
    # load image
    assert os.path.exists(
        cfg.image_path
    ), f"Could not find image {cfg.image_path}!"
    rgba = cv2.cvtColor(
        cv2.imread(cfg.image_path, cv2.IMREAD_UNCHANGED), cv2.COLOR_BGRA2RGBA
    )
    rgba = (
        cv2.resize(
            rgba, (width, height), interpolation=cv2.INTER_AREA
        ).astype(np.float32)
        / 255.0
    )
    rgb = rgba[..., :3]
    ...
    print(
        f"[INFO] single image dataset: load image {cfg.image_path} {rgb.shape}"
    )

    # load depth
    if cfg.requires_depth:
        depth_path = cfg.image_path.replace("_rgba.png", "_depth.png")
    ...
    # load normal
    if cfg.requires_normal:
        normal_path = cfg.image_path.replace("_rgba.png", "_normal.png")
        assert os.path.exists(normal_path)
        normal = cv2.imread(normal_path, cv2.IMREAD_UNCHANGED)
        normal = cv2.resize(
            normal, (width, height), interpolation=cv2.INTER_AREA
        )
        ...
        normal = None
    return rgb, mask, depth, normal, rgba
...
class SingleImageDataBase:
    ...
    def load_images(self):
        ...
        # load image
        self.rgb, self.mask, self.depth, self.normal = load_images(self.cfg, (self.width, self.height))[:-1]
...
class SingleImageIterableDataset(IterableDataset, SingleImageDataBase, Updateable):
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        if self.cfg.eval_bg_color is not None:
            batch.update({'bg_color': torch.as_tensor(self.cfg.eval_bg_color, dtype=torch.float)/255})
...
class SingleImageDataModule(pl.LightningDataModule):
    ...
    def general_loader(self, dataset, batch_size, num_workers, collate_fn=None) -> DataLoader:
        return DataLoader(
            dataset, num_workers=num_workers, batch_size=batch_size, collate_fn=collate_fn
        )
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        return self.general_loader(
            ...
            num_workers=self.cfg.num_workers, collate_fn=self.train_dataset.collate
        )

    def val_dataloader(self) -> DataLoader:
        return self.general_loader(
            self.val_dataset, batch_size=1, num_workers=0, 
        )
        # return self.general_loader(self.train_dataset, batch_size=None, collate_fn=self.train_dataset.collate)

    def test_dataloader(self) -> DataLoader:
        return self.general_loader(
            self.test_dataset, batch_size=1, num_workers=0,
        )
    ...
    def get_all_train_iters(self,):
        dataiters = []
        for step in self.train_dataset.resolution_milestones:
            self.train_dataset.update_step(0, step)
            train_loader = self.train_dataloader()
            dataiters.append(iter(train_loader))
        return DataIters(dataiters, self.train_dataset.resolution_milestones)

class DataIters:
    '''
    class for storing all data iters with different resolution depending on global_steps
    '''
    def __init__(self, dataiters, milestones):
        self.dataiters = dataiters
        self.milestones = milestones
    def get_current_iter(self, step):
        ind = bisect.bisect_right(self.milestones, step) - 1
        return self.dataiters[ind]
