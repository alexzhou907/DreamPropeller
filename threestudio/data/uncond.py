...
class RandomCameraDataset(Dataset):
    def __init__(self, cfg: Any, split: str, *args, **kwargs) -> None:
        super().__init__()
        self.cfg: RandomCameraDataModuleConfig = cfg
        self.split = split
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.eval_bg_color is not None:
            ret.update({'bg_color': torch.as_tensor(self.cfg.eval_bg_color, dtype=torch.float)/255})
...
class RandomCameraDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self, num_workers=0) -> DataLoader:
        return self.general_loader(
            self.train_dataset, batch_size=None,num_workers=num_workers, collate_fn=self.train_dataset.collate
        )

    def val_dataloader(self) -> DataLoader:
        return self.general_loader(
            self.val_dataset, batch_size=1, num_workers=0, collate_fn=self.val_dataset.collate
        )
        # return self.general_loader(self.train_dataset, batch_size=None, collate_fn=self.train_dataset.collate)

    def test_dataloader(self) -> DataLoader:
        return self.general_loader(
            self.test_dataset, batch_size=1, num_workers=0, collate_fn=self.test_dataset.collate
        )
    ...
    def get_all_train_iters(self,):
        dataiters = []
        assert self.cfg.num_workers > 0, 'Need preload., otherwise the resolution is the last one due to update_step'
        for step in self.train_dataset.resolution_milestones:
            self.train_dataset.update_step(0, step)
            train_loader = self.train_dataloader(self.cfg.num_workers)
