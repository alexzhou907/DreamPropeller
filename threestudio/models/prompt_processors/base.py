...
def side_view(s):
    return f"side view of {s}"


def side_view_back(s):
    return f"{s}, side view"

def identity(s):
    return s

def side_threshold(ele, azi, dis):
    return torch.ones_like(ele, dtype=torch.bool)


def front_view(s):
    return f"front view of {s}"

def front_view_back(s):
    return f"{s}, front view"
def front_threshold(ele, azi, dis, front_threshold):
    return (
                shift_azimuth_deg(azi) > - front_threshold
            ) & (shift_azimuth_deg(azi) < front_threshold)

def back_view(s):
    return f"backside view of {s}"

def back_view_back(s):
    return f"{s}, backside view"
def back_threshold(ele, azi, dis, back_threshold):
    return (
                shift_azimuth_deg(azi) > 180 -  back_threshold
            )  | (shift_azimuth_deg(azi) < -180 +  back_threshold)

def overhead_view(s):
    return f"overhead view of {s}"
def overhead_view_back(s):
    return f"{s}, overhead view"


def overhead_threshold(ele, azi, dis, overhead_threshold):
        ...
        if self.cfg.view_dependent_prompt_front:
            self.directions = [
                ...
                DirectionConfig(
                    ...
                    front_view,
                    identity,
                    partial(front_threshold, front_threshold=self.cfg.front_threshold),
                ),
                DirectionConfig(
                    "back",
                    back_view,
                    identity,
                    partial(back_threshold, back_threshold=self.cfg.back_threshold),
                ),
                DirectionConfig(
                    "overhead",
                    overhead_view,
                    identity,
                    partial(overhead_threshold, overhead_threshold=self.cfg.overhead_threshold),
                ),
            ]
        else:
            self.directions = [
