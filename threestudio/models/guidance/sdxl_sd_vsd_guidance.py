...
@threestudio.register("sdxl-sd-vsd-guidance")
class SDXLVSDGuidance(BaseModule):
    @dataclass
    ...
    def sample(
        ...
        **kwargs,
        ...
        generator = torch.Generator(device=elevation.device).manual_seed(seed)
    ...
    @torch.cuda.amp.autocast(enabled=False)
    def encode_images_lora(
        self, imgs: Float[Tensor, "B 3 512 512"]
    ) -> Float[Tensor, "B 4 64 64"]:
        input_dtype = imgs.dtype
        imgs = imgs * 2.0 - 1.0
        posterior = self.vae_lora.encode(imgs.to(self.vae_lora.dtype)).latent_dist
        latents = posterior.sample() * self.vae_lora.config.scaling_factor
        return latents.to(input_dtype)
    ...
    def compute_grad_vsd(
        ...
        with torch.no_grad():
            ...
            add_time_ids = torch.cat([add_time_ids]*2, dim=0).to(text_embeddings.device).repeat(B , 1)
            added_cond_kwargs = {"text_embeds": pooled_text_embeddings_vd, "time_ids": add_time_ids}
            # add noise
            ...
            noise = noise.unfold(2, 2, 2)[...,0].unfold(3, 2, 2)[...,0]
            latents_noisy = self.scheduler.add_noise(latents_lora, noise, t)
            # pred noise
            latent_model_input = torch.cat([latents_noisy] * 2, dim=0)
        ...
        grad_pretrain = w * (noise_pred_pretrain)
        grad_lora = - w * noise_pred_est
        return grad_pretrain, grad_lora
    ...
    def get_latents_lora(
        ...
        if rgb_as_latents:
            ...
            # encode image into latents with vae
            latents = self.encode_images_lora(rgb_BCHW_512)
        return latents
    ...
    def forward(
        ...
        **kwargs,
        ...
        latents_lora = self.get_latents_lora(rgb_BCHW, rgb_as_latents=rgb_as_latents)

        # view-dependent text embeddings
        text_embeddings_vd, pooled_text_embeddings_vd = prompt_utils['base'].get_text_embeddings(
            elevation,
            azimuth,
            camera_distances,
            view_dependent_prompting=self.cfg.view_dependent_prompting,
        ...
        # input text embeddings, view-independent
        text_embeddings = prompt_utils['lora'].get_text_embeddings(
                elevation, azimuth, camera_distances, view_dependent_prompting=False
            )
        ...
        grad_pretrain, grad_lora = self.compute_grad_vsd(
            latents, latents_lora, text_embeddings_vd, pooled_text_embeddings_vd, text_embeddings, camera_condition
        )

        grad_pretrain = torch.nan_to_num(grad_pretrain)
        ...
        target_pretrain = (latents - grad_pretrain).detach()
        loss_vsd = 0.5 * F.mse_loss(latents, target_pretrain, reduction="sum") / batch_size
        target_lora = (latents_lora - grad_lora).detach()
        loss_vsd += 0.5 * F.mse_loss(latents_lora, target_lora, reduction="sum") / batch_size

        loss_lora = self.train_lora(latents_lora, text_embeddings, camera_condition)
