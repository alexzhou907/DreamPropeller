...
class DeepFloydGuidance(BaseObject):
    ...
    # def set_unet(self):
    #     self.unet = {
    #         torch.device(f'cuda:{i}'): copy.deepcopy(self.unet).to(torch.device(f'cuda:{i}')) for i in range(torch.cuda.device_count())
    #     }
    #     self.alphas = {
    #         torch.device(f'cuda:{i}'): copy.deepcopy(self.alphas).to(torch.device(f'cuda:{i}')) for i in range(torch.cuda.device_count())
    #     }
    ...
    def get_noise_pred(
        ...
        if use_perp_neg:
            ...
            noise_pred = self.forward_unet(
                latent_model_input,
                torch.cat([t.reshape(1)] * 4).to(latents_noisy.device),
                encoder_hidden_states=text_embeddings,
            )  # (4B, 6, 64, 64)
