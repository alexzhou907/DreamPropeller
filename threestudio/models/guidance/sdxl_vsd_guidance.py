...
@threestudio.register("sdxl-vsd-guidance")
class SDXLVSDGuidance(BaseModule):
    ...
    def configure(self, device) -> None:
        ...
        if self.cfg.half_precision_weights:
            ...
            pipe_kwargs['vae'] = pipe_lora_kwargs['vae']= vae

        @dataclass
        class SubModules:
            pipe: DiffusionPipeline
            pipe_lora: DiffusionPipeline
        ...
        if not self.single_model:
            ...
            del  self.pipe_lora.text_encoder_2
        cleanup()
    ...
    def compute_grad_vsd(
        ...
        with torch.no_grad():
            ...
            pooled_text_embeddings_cond, _ = pooled_text_embeddings.chunk(2)
            added_cond_kwargs = {"text_embeds": torch.cat([pooled_text_embeddings_cond] * 2), "time_ids": add_time_ids}
    ...
    def train_lora(
        ...
        add_time_ids = add_time_ids.to(text_embeddings.device).repeat(self.cfg.lora_n_timestamp_samples , 1)
        added_cond_kwargs = {"text_embeds": pooled_text_embeddings_cond.repeat(self.cfg.lora_n_timestamp_samples, 1), "time_ids": add_time_ids}
    ...
    def forward(
        ...
        **kwargs,
        ...
        # view-dependent text embeddings
        text_embeddings_vd, pooled_text_embeddings_vd = prompt_utils.get_text_embeddings(
            elevation,
            azimuth,
            camera_distances,
            view_dependent_prompting=self.cfg.view_dependent_prompting,
        ...
        grad = self.compute_grad_vsd(
            latents, text_embeddings_vd, pooled_text_embeddings_vd, text_embeddings, pooled_text_embeddings, camera_condition
        )
        ...
        loss_lora = self.train_lora(latents, text_embeddings, pooled_text_embeddings, camera_condition)
