...
class BaseSystem(pl.LightningModule, Updateable, SaverMixin):
    ...
    def __init__(self, cfg, device=get_device(), resumed=False, configure=True) -> None:
        super().__init__()
        ...
        if configure:
            self.base_configure(device)
       
    def base_configure(self, device):
...
class BaseLift3DSystem(BaseSystem):
    ...
    def configure(self, device) -> None:
        if (
            ...
            prev_geometry = threestudio.find(prev_system_cfg.geometry_type)(
                prev_geometry_cfg, device=device
            )
            state_dict, epoch, global_step = load_module_weights(
            ...
            prev_geometry = prev_geometry.to(device)
            ...
            self.geometry = threestudio.find(self.cfg.geometry_type)(self.cfg.geometry, device=device)

        self.material = threestudio.find(self.cfg.material_type)(self.cfg.material, device=device)
        self.background = threestudio.find(self.cfg.background_type)(
            self.cfg.background, device=device
        )
    ...
    #     getattr(torch.optim,  self.cfg.optimizer.name)(params, ** self.cfg.optimizer.args)

    def clone(self, device, other=None):

        if other is None:
            # make sure to clone after on_fit_start
            other = type(self)(self.cfg, device=device, configure=False)
            other.geometry = copy.deepcopy(self.geometry)
            other.material = copy.deepcopy(self.material)
            other.background = copy.deepcopy(self.background)
            other.renderer = threestudio.find(self.cfg.renderer_type)(
                    self.cfg.renderer,
            ...
            #     other.prompt_processor = self.prompt_processor
            # if hasattr(self, "prompt_utils"):
            #     other.prompt_utils = self.prompt_utils
            # if hasattr(self, "guidance"):
            #     other.guidance = self.guidance
            # other.geometry = type(self.geometry)(self.cfg.geometry)
            
            # other.background = type(self.background)(
            #     self.cfg.background
            # )

        with torch.no_grad():
            for parameter_to, parameter_from in zip(other.geometry.parameters(), self.geometry.parameters()):
                parameter_to.data = parameter_from.data.clone()
                
            for parameter_to, parameter_from in zip(other.material.parameters(), self.material.parameters()):
                parameter_to.data = parameter_from.data.clone()
                
            for parameter_to, parameter_from in zip(other.background.parameters(), self.background.parameters()):
                parameter_to.data = parameter_from.data.clone()
            for parameter_to, parameter_from in zip(other.renderer.parameters(), self.renderer.parameters()):
                parameter_to.data = parameter_from.data.clone()
                
            
        if self.renderer.cfg.get('grid_prune', False):
            other.renderer.estimator.occs = self.renderer.estimator.occs.clone()
            other.renderer.estimator.binaries = self.renderer.estimator.binaries.clone()


        # if isinstance(self.geometry, ImplicitSDF):
        #     other.geometry.finite_difference_normal_eps = self.geometry.finite_difference_normal_eps
        return other.to(device, non_blocking=True)
    def get_aux_params(self):
        aux = {}
        if self.renderer.cfg.get('grid_prune', False):
            aux['occs'] =  self.renderer.estimator.occs
            # aux['binaries'] =   self.renderer.estimator.binaries
        return aux
    def get_params(self,include_occs=True, include_binaries=False):
        # myparams = {**dict(self.geometry.named_parameters()), **dict(self.material.named_parameters()) , **dict(self.background.named_parameters())}
        # if hasattr(self.guidance, 'unet_lora'):
        #     myparams.update(dict(self.guidance.unet_lora.named_parameters()))
        myparams = list(self.geometry.parameters())+list(self.material.parameters())+list(self.background.parameters())+list(self.renderer.parameters())
            
        if self.renderer.cfg.get('grid_prune', False) and include_occs:
            myparams += [self.renderer.estimator.occs]#, self.renderer.estimator.binaries]

        if self.renderer.cfg.get('grid_prune', False) and include_binaries:
            myparams += [self.renderer.estimator.binaries]#, self.renderer.estimator.binaries]
        
        return myparams
    def set_renderer_binaries(self):
        if self.renderer.cfg.get('grid_prune', False):
            thre = torch.clamp(self.renderer.estimator.occs[self.renderer.estimator.occs >= 0].mean(), max=0.01)
            self.renderer.estimator.binaries = (self.renderer.estimator.occs > thre).view(self.renderer.estimator.binaries.shape)
    def set_aux_params(self, aux):
        if self.renderer.cfg.get('grid_prune', False):
            self.renderer.estimator.occs = aux['occs'].to(self.renderer.estimator.occs.device, non_blocking=True)
            # self.renderer.estimator.binaries = aux['binaries'].to(self.renderer.estimator.binaries.device, non_blocking=True)
    def set_params(self, params, clone=True):
        myparams = self.get_params(include_occs=True, include_binaries=True)
        # if isinstance(self.geometry, ImplicitSDF):
        #     self.geometry.finite_difference_normal_eps = aux['finite_difference_normal_eps']
        for p, q in zip(myparams, params):
            if clone: 
                p.data = q.data.clone().to(p.device, non_blocking=True)
            else:
                p.data = q.to(p.device, non_blocking=True)
        # for p, q in zip(myparams[-2:], params[-2:]):
        #     p.data = q.data.clone().to(p.device, non_blocking=True)
        # self.set_renderer_binaries()

    def set_grads_from_grads(self, grads):
        # myparams = {**dict(self.geometry.named_parameters()), **dict(self.material.named_parameters()) , **dict(self.background.named_parameters())}
        # if hasattr(self.guidance, 'unet_lora'):
        #     myparams.update(dict(self.guidance.unet_lora.named_parameters()))
        myparams = self.get_params(include_occs=False)
        # assert len(myparams) == len(grads)
        for p, grad in zip(myparams, grads):
            if grad is not None:
                p.grad = grad.to(p.device, non_blocking=True)
                

    def compute_error_from_system(self, other):
        myparams = self.get_params(include_occs=False)
        otherparams = other.get_params(include_occs=False)
        with torch.no_grad():
            error = 0.0
            total_num = 0
