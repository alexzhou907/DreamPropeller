...
class Zero123(BaseLift3DSystem):
    ...
    def on_fit_start(self, device) -> None:
        super().on_fit_start()
        # no prompt processor
        self.guidance = threestudio.find(self.cfg.guidance_type)(self.cfg.guidance, device)

        # visualize all training images
        # all_images = dataloader.dataset.get_all_images()
    ...
    def validation_step(self, batch, batch_idx, texts):
        out = self(batch)
        self.save_image_grid(
            f"it{batch_idx}-val/{batch['index'][0]}.png",
            (
                [
                    {
                        "type": "rgb",
    ...
    def on_validation_epoch_end(self, step):
        filestem = f"it{step}-val"
        ...
        shutil.rmtree(
            os.path.join(self.get_save_dir(), f"it{step}-val")
        )
    ...
    def on_test_epoch_end(self, step):
        ...
        # shutil.rmtree(
        #     os.path.join(self.get_save_dir(), f"it{step}-test")
        # )
